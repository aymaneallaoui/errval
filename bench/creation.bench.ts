...
// and slow rows: errval skips it unless you ask, everything built on `new Error` pays it.
...
group("create one domain error", () => {
  summary(() => {
    bench("errval TaggedError", () => do_not_optimize(new NotFound({ id: nextId() }))).baseline(
      true,
    )
    bench("errval TaggedError, stack: true", () => do_not_optimize(new Traced({ id: nextId() })))
    bench("errval TaggedError, native: true", () =>
...
group("create one failed result", () => {
  summary(() => {
    bench("errval fail(new NotFound)", () =>
